...
func newServer(store store.Store, port int, cancel context.CancelFunc, logger *slog.Logger) *server {
	...
	srv.Handler = otelhttp.NewHandler(srv.Handler, "http.server")
	...
	mux.Handle("GET /debug/pprof/heap", s.authMiddleware(pprof.Handler("heap")))
...
func (s *server) start() error {
	...
	netAddr := ln.Addr()
	s.logger.Debug(fmt.Sprintf("Linko is running on http://localhost:%d", netAddr.(*net.TCPAddr).Port))
